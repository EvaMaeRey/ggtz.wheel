...
At the same time, we'll maintain a comforting safe distance to being productive: talk about packaging practices, and write a package, and make a printable timezone wheel, plan the ggdibbler meeting, talk personal taste for ggplot2 wrappers -- and try to figure out my wrapping taste even is, maybe think about an accompanying app for the package. Check in with the ggtime crew to see if there are better ways of tackling this!
...
And then somewhere along the line, what I thought could be a helpful visual was a timezone wheel, which I sketched out here - this spinnable version made it feel like I could not only communicate timezones for a planned meeting, but also pretty easily check what set of local times might work! ☺️
